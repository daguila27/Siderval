    ...
    if(verificar(req.session.userData)){
        ...
        if(input.agrupar == 'true'){
            query = "SELECT SUM(produccion.`1`) as `1`,SUM(produccion.`2`) as `2`,SUM(produccion.`3`) as `3`,SUM(produccion.`4`) as `4`,SUM(produccion.`5`) as `5`,"
                + "SUM(produccion.`6`) as `6`,SUM(produccion.`7`) as `7`,SUM(produccion.`8`) as `8`, GROUP_CONCAT(produccion.idproduccion separator ' - ') as idproduccion, COALESCE(SUM(produccion_history.enviados),0) as trats, GROUP_CONCAT(opQuery.idordenfabricacion separator ' - ') as numordenfabricacion, GROUP_CONCAT(fabricaciones.idfabricaciones separator ' - ') as idfabricaciones, GROUP_CONCAT(produccion.idordenproduccion separator ' - ') as idordenproduccion, sum(produccion.cantidad) as cantidad, material.detalle,  opQuery.tok ,opQuery.idordenfabricacion"
